  ...
  const breakdownText =
    expenseBreakdown.length > 0
      ? expenseBreakdown
          .map(
            (c) =>
              `- ${c.category}: ${currency} ${(c.amount || 0).toFixed(2)}`
          )
          .join('\n')
  ...
  const trendText =
    previousMonths.length > 0
      ? previousMonths
          .map(
            (m) =>
              `- ${m.month}: Income ${currency} ${(m.income || 0).toFixed(2)}, Expenses ${currency} ${(m.expenses || 0).toFixed(2)}`
          )
          .join('\n')
...
Total Income (this month): ${currency} ${(totalIncome || 0).toFixed(2)}
Total Expenses (this month): ${currency} ${(totalExpenses || 0).toFixed(2)}
Savings Rate: ${(savingsRate || 0).toFixed(1)}%
...
`;
  ...
  try {
    ...
    throw new Error(
        ...
        'Failed to generate monthly insight.'
    );
}
};
...
export const analyzeBudgetList = async ({ budgets, currency = 'USD' }) => {
    ...
    const prompt = `
You're a personal finance assistant. Analyze each budget below and provide a one-sentence assessment.
